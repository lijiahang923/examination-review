...
# 第一部分：数据库原理及应用——核心复习资料

## 第一章：数据库系统概论
**1. 基本概念**
*   **DB (Database)**: 长期存储在计算机内、有组织的、可共享的大量数据的集合。
*   **DBMS (Database Management System)**: 位于用户与操作系统之间的一层数据管理软件（如SQL Server, Oracle, MySQL）。主要功能包括数据定义、操纵、控制、维护。
*   **DBS (Database System)**: **DBS = DB + DBMS + 应用程序 + DBA (数据库管理员)**。
    *   *考点关联*：DBS包含DBMS和DB。
...
# 第一部分：地基篇——搞懂数据库是干嘛的
**(对应第1章：认识数据库 & 第2章：数据模型)**

### 1. 为什么要用数据库？（第1章核心）
想象一下，在没有电脑的时代，你要管理一个学校的学生信息：
...
**★ 必考点：三级模式与两级映像（这是数据库的“骨架”）**
为了让改数据库结构时不影响程序，数据库分了三层：
1.  **外模式（用户模式）**：**用户看到的**。比如你是学生，你只能看到成绩表；老师能看到工资表。一个数据库可以有多个外模式。
2.  **模式（逻辑模式/概念模式）**：**数据库的整体逻辑**。这是数据库的“蓝图”，比如规定学生表有学号、姓名。**一个数据库只有一个模式**。
3.  **内模式（物理模式）**：**存在硬盘上的**。数据具体存在哪个磁道、用什么格式存。**一个数据库只有一个内模式**。

**两级映像（保证独立性）：**
*   **外模式/模式映像** $\to$ 保证**逻辑独立性**（表结构变了，你的程序不用改）。
*   **模式/内模式映像** $\to$ 保证**物理独立性**（存硬盘的位置变了，表结构不用改）。

### 2. 怎么把现实变成电脑里的表？（第2章核心）
...
# 第一章：认识数据库 (Introduction to DBS)

这一章主要解决一个问题：**我们为什么要用数据库，以及数据库系统长什么样。**
...
# 第3章：关系数据模型 (Relational Data Model)

**核心地位：** 这是现代数据库（如SQL Server, MySQL）的数学基础。不懂这一章，后面的SQL查询就是瞎写。

## 1. 关系数据结构 (Relational Structure)
...
## 第三章：数据库设计 (六个阶段)
1.  **需求分析**: 收集用户需求（数据流图、数据字典）。
2.  **概念结构设计**: 产出 **E-R图**。(*试卷考点*)
3.  **逻辑结构设计**: 将E-R图转换为关系模式（表结构），进行规范化。
4.  **物理结构设计**: 选择存储结构、索引、存取方法。
5.  **数据库实施**: 编写SQL、载入数据。
6.  **运行与维护**.

# 数据库原理及应用 —— 高分复习全书 (Part 1)

## 🚀 第一模块：关系数据库理论深度剖析 (难点攻克)

这部分通常是选择题和填空题的“失分重灾区”，务必彻底理解，而非死记硬背。
...
# 第5章：数据库设计 (Database Design)

这一章是简答题和设计题（画图）的大本营。

## 1. 设计的六个阶段 (The 6 Steps)

请务必背诵这个顺序，考试常考排序或填空。

1.  **需求分析**：(做什么？) 产出数据流图(DFD)和数据字典。
2.  **概念结构设计**：(画草图) **产出E-R图**。不依赖具体的电脑软件。
3.  **逻辑结构设计**：(转成表) 把E-R图变成关系模式，做规范化(3NF)。
4.  **物理结构设计**：(存哪里？) 索引、存储路径。
5.  **数据库实施**：(写代码) 建库、载入数据。
6.  **运行与维护**。
...
## 第四章：SQL Server 基础
**1. 系统数据库**
*   **master**: 记录SQL Server系统的所有系统级信息（登录、配置等），最重要。
*   **model**: 创建新数据库的模板。
*   **msdb**: 用于代理服务调度作业、报警等。
*   **tempdb**: 临时数据库，重启后数据丢失。
    *   *注意*: AdventureWorks 是示例数据库，不是系统数据库。
...
# 第三部分：工具操作篇——SQL Server 怎么用
**(对应第6-8、10章：SQL Server管理)**
